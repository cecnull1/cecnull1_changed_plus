...
import com.github.cecnull1.cecnull1_changed_plus.capability.HAState
import com.github.cecnull1.cecnull1_changed_plus.capability.HAStateProvider
...
import com.github.cecnull1.cecnull1_changed_plus.utils.TransfurData.Companion.toTransfurDataOrNull
...
//        }
    ...
    fun onLivingDeath(event: LivingDeathEvent) {
        ...
        player?.let {
            if (!player.isPlayerTransfurred || !player.level.gameRules.getBoolean(ChangedGameRules.RULE_KEEP_FORM)) {
...
//                if (player is ServerPlayer) player.sendHealthUpdate()
    ...
    fun onEntityPickup(event: EntityItemPickupEvent) {
        event.player?.ifPlayerTransfurred {
            if (it.`is`(ModTransfurVariant.SOUL_TRANSFUR_VARIANT)) {
                event.isCanceled = true
            }
        }
    ...
    fun onPlayerLogin(event: PlayerEvent.PlayerLoggedInEvent) {
        ...
        if (!event.player.level.isClientSide) {
            NetworkHandler.sendToClient(event.player)
        }
    }
}
